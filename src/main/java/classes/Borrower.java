...
public class Borrower extends User {
  ...
  public void updateProfile(String fullName) {
    ...
    String sql = "UPDATE Borrowers SET ho_ten = ? WHERE id = ?";
  ...
  public boolean borrowBookByIsbn(String isbn, int quantity) {
    if (quantity <= 0) {
      System.out.println("Số lượng sách mượn phải lớn hơn 0");
      return false;
    }
  ...
  private boolean addBorrowedBookRecord(int borrowerId, String title, String isbn, int quantity) {
    String sql =
        "INSERT INTO BorrowedBookRecord (borrower_id,title, isbn, quantity, borrowed_date) "
            + "VALUES (?, ?, ?, ?,?)";
  ...
  public boolean returnBook(int recordId) {
    ...
    if (book != null) {
      ...
      return deleteBorrowedBookRecord(recordId);// Xóa bản ghi mượn sách
    }
    return false;
  }
